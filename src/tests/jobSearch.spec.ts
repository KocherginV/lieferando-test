...
test.beforeEach(async ({ page }) => {
  ...
  test.describe('Job search tests', () => {
    test('Search results can be filtered by country', async({page}) => {
        ...
        let locationsTextValues = await searchResultsPage.locationList.allInnerTexts();
    ...
    test('Search results count is returned correctly, after multiple fiters applied', async({page}) =>{
        const careersHomePage = new CareersHomePage(page);
        const searchResultsPage = new SearchResultsPage(page);
        const helper = new Helper();
        ...
        let filteredJobCount = await searchResultsPage.jobsInFilter('CategoryBody', 'Sales').textContent();
        expect(searchResultsPage.topMenuJobCount).toHaveText(helper.extractNumber(filteredJobCount));
        await searchResultsPage.countryButton.click();
        await searchResultsPage.countryItemCheckbox('Germany').check();
        expect(searchResultsPage.appliedCountryTag).toHaveText('Germany');
        filteredJobCount = await searchResultsPage.jobsInFilter('CountryBody', 'Germany').textContent();
        await expect(searchResultsPage.topMenuJobCount).toHaveText(helper.extractNumber(filteredJobCount));
    });
});
